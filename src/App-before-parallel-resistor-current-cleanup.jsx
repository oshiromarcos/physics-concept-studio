...
function ParallelDiagram({ supplyVoltage, resistors, rows, totalCurrent }) {
  const branchPositions = resistors.length === 3 ? [330, 500, 670] : [390, 590];
  ...
  return (
    <svg viewBox="0 0 850 390" width="100%" height="100%">
      ...
      <line x1="267" y1="90" x2="735" y2="90" stroke="#30271e" strokeWidth="8" strokeLinecap="round" />
      <text x="208" y="130" fill="#6f624d" fontSize="13">
        Itotal = {totalCurrent.toFixed(2)} A
      </text>

      <path d="M135 168 V300 H735" fill="none" stroke="#30271e" strokeWidth="8" strokeLinecap="round" />

      {branchPositions.map((x, index) => (
        <g key={index}>
          <path d={`M${x} 90 V122`} stroke="#30271e" strokeWidth="8" strokeLinecap="round" />

          <circle cx={x} cy="150" r="23" fill="#fff" stroke="#30271e" strokeWidth="4" />
          <text x={x} y="158" textAnchor="middle" fill="#30271e" fontSize="23" fontWeight="900">A</text>
          <text x={x} y="120" textAnchor="middle" fill="#6f624d" fontSize="13">
            I{index + 1} = {rows[index].current.toFixed(2)} A
          </text>

          <path d={`M${x} 173 V190`} stroke="#30271e" strokeWidth="8" strokeLinecap="round" />

          <rect x={x - 44} y="190" width="88" height="48" rx="10" fill="#fff" stroke="#30271e" strokeWidth="4" />
          <text x={x} y="218" textAnchor="middle" fill="#30271e" fontSize="13" fontWeight="800">
            R{index + 1} = {resistors[index].toFixed(1)} Ω
          </text>

          <path d={`M${x} 238 V300`} stroke="#30271e" strokeWidth="8" strokeLinecap="round" />
        </g>
      ))}

      <path d="M170 90 H205" stroke="#30271e" strokeWidth="3" markerEnd="url(#arrowParallel)" />
      <path d="M285 90 H330" stroke="#30271e" strokeWidth="3" markerEnd="url(#arrowParallel)" />
      <path d="M610 300 H560" stroke="#30271e" strokeWidth="3" markerEnd="url(#arrowParallel)" />

      <text x="445" y="340" fill="#6f624d" fontSize="14">
