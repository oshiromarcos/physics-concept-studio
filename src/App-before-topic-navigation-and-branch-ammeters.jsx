...
function ParallelDiagram({ supplyVoltage, resistors, rows }) {
  ...
  return (
    <svg viewBox="0 0 850 390" width="100%" height="100%">
      ...
      {/* Correct topology:
          Ammeter is in series before the parallel network.
          There is NO ammeter branch connecting the top and bottom rails. */}
      ...
      {/* Parallel branches: only resistors connect the top and bottom rails */}
      {branchPositions.map((x, index) => (
        <g key={index}>
          <path d={`M${x} 90 V145`} stroke="#30271e" strokeWidth="8" strokeLinecap="round" />
          <rect x={x - 42} y="145" width="84" height="56" rx="10" fill="#fff" stroke="#30271e" strokeWidth="4" />
          <text x={x} y="137" textAnchor="middle" fill="#30271e" fontSize="14" fontWeight="800">
            R{index + 1} = {resistors[index].toFixed(1)} Ω
          </text>
          <text x={x} y="179" textAnchor="middle" fill="#6f624d" fontSize="13">
            I = {rows[index].current.toFixed(2)} A
          </text>
          <path d={`M${x} 201 V300`} stroke="#30271e" strokeWidth="8" strokeLinecap="round" />
        </g>
      ))}
      ...
      <text x="455" y="340" fill="#6f624d" fontSize="14">
